<template>
  <AppLayout>
    ...
    >
        ...
        <div class="section-heading">
          <div>
            <h2 class="section-title">
              {{ t('jenkinsRecords.filtersTitle') }}
            </h2>
            <p class="section-copy">
              {{ t('jenkinsRecords.filtersSubtitle') }}
            </p>
          </div>
        </div>
        <div class="grid gap-4 lg:grid-cols-[1fr_1fr_auto]">
          ...
          <select v-model="filterStatus" @change="loadRecords" class="input">
            ...
            <option value="pending">
              {{ t('jenkinsRecords.status.pending') }}
            </option>
            <option value="running">
              {{ t('jenkinsRecords.status.running') }}
            </option>
            <option value="success">
              {{ t('jenkinsRecords.status.success') }}
            </option>
            <option value="failure">
              {{ t('jenkinsRecords.status.failure') }}
            </option>
            <option value="aborted">
              {{ t('jenkinsRecords.status.aborted') }}
            </option>
          </select>
          <BaseButton variant="secondary" @click="clearFilters">{{
            t('jenkinsRecords.clearFilters')
          }}</BaseButton>
        </div>
      </section>
          ...
          <table class="workspace-table">
            <thead>
              <tr>
                ...
                <th>
                  {{ t('jenkinsRecords.notificationSummary.columnHeader') }}
                </th>
              ...
              <tr v-for="record in records" :key="record.id">
                ...
                <td>
                  <div v-if="hasBuildProgress(record)" class="min-w-[10rem]">
                    <div class="flex items-center justify-between gap-3">
                      ...
                      <span
                        class="max-w-[9rem] truncate text-xs text-slate-500"
                      >
                ...
                <td>
                  <div class="max-w-[18rem]">
                    <div class="font-medium text-slate-800">
                      {{
                        record.notification_result?.summary ||
                        t('jenkinsRecords.notificationSummary.pendingFallback')
                      }}
                    </div>
                    <div class="mt-1 text-xs text-slate-500">
                      {{
                        t('jenkinsRecords.notificationSummary.channelSummary', {
                          emailCount:
                            record.notification_result?.emails?.length || 0,
                          webhookCount:
                            record.notification_result?.webhooks?.length || 0
                        })
                      }}
                    </div>
                  </div>
                ...
                <td>
                  <div class="flex flex-wrap gap-3">
                    ...
                    >
                      {{
                        refreshing === record.id
                          ? t('jenkinsRecords.refreshing')
                          : t('jenkinsRecords.refresh')
                      }}
                    </button>
                    <button
                    ...
                    >
                      {{
                        t('jenkinsRecords.artifactCount', {
      ...
      <div
        v-if="totalPages > 1"
        class="surface-panel-strong flex items-center justify-between px-5 py-4"
      >
        <BaseButton
          variant="secondary"
        ...
        <span class="text-sm font-medium text-slate-500">{{
          t('jenkinsRecords.pageIndicator', {
            current: currentPage,
            total: totalPages
          })
        }}</span>
      ...
      <BaseModal
        :show="showArtifactsModal"
        :title="t('jenkinsRecords.artifactsTitle')"
        @close="showArtifactsModal = false"
      >
        <div class="space-y-3">
          <div
          ...
          >
            ...
            <div class="mt-1 break-words font-mono text-xs text-slate-500">
              {{ artifact.path }}
            </div>
          </div>
        </div>
        <template #footer>
          <div class="flex w-full justify-end">
            <BaseButton @click="showArtifactsModal = false">{{
              t('common.close')
            }}</BaseButton>
          </div>
        </template>
...
function getAutoRefreshCandidates() {
  return records.value.filter(
    (record) => isActiveBuildStatus(record.status) && hasRefreshTarget(record)
  )
}
...
async function loadRecords() {
  try {
    ...
    showToast(
      t('jenkinsRecords.toast.loadRecordsFailed', { message: e.message }),
      'error'
    )
  }
}
...
async function loadEntries() {
  try {
    ...
    showToast(
      t('jenkinsRecords.toast.loadEntriesFailed', { message: e.message }),
      'error'
    )
  }
}
...
async function refreshStatus(record, { silent = false, manual = true } = {}) {
  ...
  try {
    ...
    if (manual && !silent) {
      showToast(
        t('jenkinsRecords.toast.refreshFailed', { message: e.message }),
        'error'
      )
    } else {
...
async function refreshActiveRecords() {
  ...
  const candidates = getAutoRefreshCandidates().filter(
    (record) => !activeRefreshIds.has(record.id)
  )
  ...
  try {
    await Promise.allSettled(
      candidates.map((record) =>
        refreshStatus(record, { silent: true, manual: false })
      )
    )
  } finally {
