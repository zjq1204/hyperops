...
const baseModalSource = read('../../src/components/ui/BaseModal.vue')
...
assert.match(uploadSource, /credential_type/)
assert.doesNotMatch(credentialsPageSource, /credentialAlgorithmLabel\(item\)/)
assert.doesNotMatch(credentialsPageSource, /shortFingerprint\(item/)
assert.doesNotMatch(credentialsPageSource, /credentialValidationKey\(item\)/)
assert.match(uploadSource, /CredentialValidationPanel/)
assert.match(uploadSource, /:z-index="90"/)
assert.match(baseModalSource, /zIndex/)
